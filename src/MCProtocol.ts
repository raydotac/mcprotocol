...
export class MCProtocol {
  ...
  private options: Required<MCProtocolOptions>
  ...
  async writeRegister(device: string, address: number, value: number): Promise<void> {
    await this.batchWriteWordUnits([{ device, address, value }])
  }

  async batchReadWordUnits(addresses: Array<{ device: string, address: number, count?: number }>): Promise<ReadResult> {
  ...
  async batchWriteWordUnits(data: Array<{ device: string, address: number, value: number }>): Promise<void> {
    if (!this.isConnected || !this.socket) {
      throw new MCProtocolError('Not connected to PLC')
    }
  ...
  private create4EReadFrame(device: string, address: number, count: number): Buffer {
  ...
  private create4EWriteFrame(device: string, address: number, values: number[]): Buffer {
    let requestData = Buffer.alloc(0)
  ...
  private encodeValue(value: number, mode: string = "short", isSigned: boolean = false): Buffer {
    let buffer: Buffer
  ...
  private makeDeviceData(device: string, address: number): Buffer {
    ...
    const deviceCode = MCProtocol.DEVICE_CODES[deviceType as keyof typeof MCProtocol.DEVICE_CODES]
