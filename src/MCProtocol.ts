...
export class MCProtocol {
  ...
  async readRegister(device: string, address: number | string): Promise<number> {
    const result = await this.batchReadWordUnits([{ device, address }]);
    const key = `${device}${this.formatAddressForKey(device, address, 0)}`;
  ...
  async batchReadWordUnits(
    addresses: Array<{ device: string; address: number | string; count?: number }>
  ): Promise<ReadResult> {
    ...
    for (const addr of addresses) {
      const normalizedAddress = this.parseAddress(addr.device, addr.address);
      ...
      for (let i = 0; i < values.length; i++) {
        const key = `${addr.device}${this.formatAddressForKey(addr.device, addr.address, i)}`;
  ...
  async batchWriteWordUnits(
    data: Array<{ device: string; address: number | string; value: number }>
  ): Promise<void> {
    if (!this.isConnected || !this.socket) {
      throw new MCProtocolError('Not connected to PLC');
    }
    for (const item of data) {
      const normalizedAddress = this.parseAddress(item.device, item.address);
      const frame = this.create4EWriteFrame(item.device, normalizedAddress, [
        item.value,
      ]);
  ...
  /**
   * Parse address from string or number, handling both decimal and hexadecimal formats
   */
  private parseAddress(device: string, address: number | string): number {
    if (typeof address === 'number') {
      return address;
    }
    // If address is a string, determine if we should parse as hex or decimal
    const deviceBase = this.getDeviceBase(device);
    return parseInt(address, deviceBase);
  }
  /**
   * Format address for result key, maintaining original format when possible
   */
  private formatAddressForKey(device: string, address: number | string, offset: number): string {
    if (typeof address === 'string') {
      const deviceBase = this.getDeviceBase(device);
      const baseAddress = parseInt(address, deviceBase);
      const newAddress = baseAddress + offset;
      if (deviceBase === 16) {
  ...
  private makeDeviceData(device: string, address: number | string): Buffer {
  ...
  // If address is already numeric (normalized earlier), use it directly.
  const deviceNum = typeof address === 'number' ? address : parseInt(address, deviceBase);
  ...
  async plcRead(registers: string[]): Promise<number[]> {
    const addresses = registers.map((reg) => {
      // Parse device and address - need to handle multi-character device codes correctly
      let device = '';
      let address = '';
      // Try to match against known device codes, longest first
      const deviceCodes = Object.keys(MCProtocol.DEVICE_CODES).sort((a, b) => b.length - a.length);
      for (const deviceCode of deviceCodes) {
        if (reg.startsWith(deviceCode)) {
          const addressPart = reg.slice(deviceCode.length);
          // Verify the remaining part is a valid address (hex or decimal)
          if (addressPart && /^[0-9A-Fa-f]+$/.test(addressPart)) {
            device = deviceCode;
            address = addressPart;
            break;
          }
      ...
      return {
        device: device,
        address: address // Keep as string to preserve format
      };
    });
    const results = await this.batchReadWordUnits(addresses);
    return registers.map(reg => {
      const match = reg.match(/^([A-Z]+)([0-9A-Fa-f]+)$/)
      if (!match) {
        throw new MCProtocolError(`Invalid register format: ${reg}`)
      }
      const device = match[1]
      const addrPart = match[2]
      const key = `${device}${this.formatAddressForKey(device, addrPart, 0)}`
